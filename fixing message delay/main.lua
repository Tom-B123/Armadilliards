...
local function processReqeusts()
    if client then
        ...
        if gameState == "lobby" then
            ...
            sendToServer("msg:"..playerName.."_".."hello server")
            table.insert(messageLog,playerName..": ".."hello server")
    ...
    elseif server then
        updateConnections()
        if gameState == "lobby" then
            sendToClient("all","msg:"..playerName.."_".."hello client")
            table.insert(messageLog,playerName..": ".."hello client")
            for i,player in ipairs(players) do
        ...
        local lastData = nil
        local quit = false
        --server reads the last 5 messages every tick, need to prevent double reading
        for i = 1,5 do
            local data = receiveFromClients()
            love.graphics.print(data,0,200+i*20)
            if data ~= lastData then
                processServerData(data)
            end
            lastData = data
        end
        love.graphics.print(lastData)
    end
end
